...
class Adhesive {
    ...
    setFeatures(handles=["close", "new"], draggable=true, persistable=true,
        ...
        if (this.parentAdhesive !== null) {
            this.drawEdge(this.parentAdhesive);
        }
    }
    ...
    addHandleURI(slot) {
        ...
        h.on("click", function() {
            let n = new Adhesive(p.board, g, p, "",
                        ["#ddddff"],
                        [32,1], [nX, nY]);
            n.setFeatures(["close","edit"], p.draggable, true, false, "URI");
            n.parentObject = p;
            p.childObjects.set(n.id, n);
...
class InstantPhoto extends Adhesive {
    ...
    setFeatures(handles=["close", "label", "uri"], draggable=true,
            persistable=true, edgeVisible=false, type) {
