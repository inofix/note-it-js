...
/// TODO: use javascript style variable naming...
...
//            alert("a: " + k)
     ...
     */
        ...
        if (typeof o === 'object') {
            alert("already there " + adhesiveKey);
            return o;
...
// it is probably time for a class Handle()
    ...
    add_handle_edit() {
        ...
        let hl = tl + 2;
        let h = this.add_handle(hl, "!");
        var n = this;
        h.on("click", function() {
            n.open_textinput(n.type, [1,0], [tl,1])
        });
    }
...
//        let l = 2;
    ...
    snuffit() {
        ...
        for (let c of this.child_objects.values()) {
            c.snuffit();
        }
        if (typeof this.parent_object !== 'undefined' &&
                    typeof this.parent_object.child_objects !== 'undefined') {
    ...
    static create(board, key) {
        ...
        if (typeof o === 'object') {
            ...
            if (typeof o["parentId"] !== 'undefined') {
                ...
                group = p.children_group;
            }
            let a = new Adhesive(board, group, key, o["color"],
                                    o["geometry"], o["position"]);
            a.set_features(o["handles"], o["draggable"], o["type"]);
    ...
    add_handle_edit() {
        ...
        h.on("click", function() {
            ...
            if (typeof n.title !== 'undefined') {
                n.open_textinput(n.title, [0.5,1], [til,1]);
            }
            if (typeof n.content !== 'undefined') {
                n.open_textinput(n.content, [0.5,2], [cl,ch]);
            }
        });
    }
...
back.append("textarea").attr("rows", 10).attr("cols", 80);
...
back.append("input").attr("type", "button")
    ...
    board.fromJSON(d3.select("#sketchboardjson").select("textarea").node().value);
});
