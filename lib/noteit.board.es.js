...
class Gallery {
    ...
    setSvgImageByUrl(Url, imageNode) {
        for (var i=0; i < this.images.length; i++) {
            if (Url == this.images[i]) {
                imageNode.attr("xlink:href", Url);
            } else {
                imageNode.text("not found");
            }
        }
    }

    setSvgImageByIndex(index, imageNode) {
        if (index < this.size && index >= 0) {
            imageNode.attr("xlink:href", this.images[index]);
        }
    }
}
...
class Adhesive {
    ...
    constructor(board, group, parentAdhesive, id="", color=["#f7ff72"], geometry=[10,10], position=[0,0]) {
        ...
        this.parentAdhesive = parentAdhesive;
        ...
        if (parentAdhesive !== null) {
            this.drawEdge(parentAdhesive);
        }
    ...
    drawEdge(adhesive) {
        let p1 = [ 0.5 * this.geometryPx[0], this.geometryPx[1] ];
    ...
    addHandleURI(slot) {
        ...
        h.on("click", function() {
            let n = new Adhesive(p.board, g, p, "",
                        ["#ddddff"],
                        [32,1], [nX, nY]);
            n.setFeatures(["close","edit"], p.draggable, true, "URI");
            n.parentObject = p;
            p.childObjects.set(n.id, n);
...
//        let l = 2;
    ...
    enableDrag() {
        ...
        var nX = this.group.node().getBoundingClientRect().left - this.parentGroup.node().getBoundingClientRect().left + this.translatePosition[0];
        var nY = this.group.node().getBoundingClientRect().top - this.parentGroup.node().getBoundingClientRect().top + this.translatePosition[1];
        ...
        this.group.call(d3.drag().on("drag", function() {
            ...
            if (n.parentAdhesive !== null) {
                n.drawEdge(n.parentAdhesive);
            }
...
class InstantPhoto extends Adhesive {
    ...
    constructor(board, group, parentAdhesive, id="", color=["#fafafa"], geometry=[16,16], position=[2,2]) {
        super(board, group, parentAdhesive, id, color, geometry, position);
        this.gallery = board.gallery;
        // crate a frame (5% top/right/left; 20% bottom)
        let pictureHeight = geometry[0] * .75;
        let pictureWidth = geometry[1] * .88;
        let pictureHeightRect = pictureHeight + .1;
        let pictureWidthRect = pictureWidth + .1;
        let pictureFrame = geometry[0] * .06;
        this.group.append("rect")
                .attr("width", pictureWidthRect + "em")
                .attr("height", pictureHeightRect + "em")
                .attr("x", pictureFrame + "em")
                .attr("y", pictureFrame + "em")
                .attr("stroke", "#555555").attr("fill", "#888888");
        this.image = this.group.append("svg:image")
                .attr("xlink:href", "")
                .attr("width", pictureWidth + "em")
                .attr("height", pictureHeight + "em")
                .attr("x", pictureFrame + "em")
                .attr("y", pictureFrame + "em");
        return this;
    }

    showPhotoByIndex(imageIndex) {
        this.gallery.setSvgImageByIndex(imageIndex, this.image);
    }

    showPhotoByUrl(imageUrl) {
        this.gallery.setSvgImageByUrl(imageUrl, this.image);
    }

    setFeatures(handles=["close", "label", "uri"], draggable=true, persistable=true, type) {
        super.setFeatures(handles, draggable, persistable, type);
    }
...
class NoteIt extends Adhesive {
    ...
    constructor(board, group, parentAdhesive, id="", color=["#f7ff72"], geometry=[10,10], position=[2,2]) {
        super(board, group, parentAdhesive, id, color, geometry, position);
        return this;
    }
