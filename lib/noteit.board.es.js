...
//        this.type.on("mouseover", () => this.focus());
    ...
    add_handle_new() {
        ...
        h.on("click", function() {
            let n = new Adhesive(p.board, p.group, p.color.slice(), p.geometry, [nX, nY]);
            n.set_features(p.handles, p.hierarchical, p.draggable, p.type.node().textContent);
            n.parent_object = p;
            p.child_objects[n.id] = n;
            p.board.adhesives[n.id] = n;
        });
    }
...
//        let l = this.geometry[0] / 2 + 1;
    ...
    toJSON() {
        ...
        if (typeof this.parent_object === 'object') {
            ...
            json += '"parentid": ' + this.parent_object + ', ';
        }
        json += '"color": [' + this.color + '], ' +
                '"geometry": [' + this.geometry + '], ' +
                '"position": [' + this.position + '], ' +
                '"type": "' + this.type.node().textContent + '" }';
        return json;
// board, group handles, hierarchical, draggable
    }
}
...
class NoteIt extends Adhesive {
    ...
    add_handle_new() {
        ...
        h.on("click", function() {
            var n = new NoteIt(p.board, p.group, p.color.slice(), p.geometry, [nX,nY]);
            n.set_features(p.handles, p.hierarchical, p.draggable, p.type.node().textContent, "Title", "...");
            n.parent_object = p;
            p.child_objects[n.id] = n;
            p.board.adhesives[n.id] = n;
        });
...
//  })
    ...
    add_title(title="Title") {
        ...
        var nt = this.title.append("foreignObject")
                    .attr("x", "6").attr("y", "16")
                    .attr("width", l + "em").attr("height", "1em")
                    .append("xhtml:div")
                    .attr("style", "width: " + l + "em; height: 1em;")
...
//        nt.on("mouseover", function() { this.focus(); });
    ...
    add_content(content="...") {
        ...
        var nt = this.content.append("foreignObject")
                    .attr("x", "6").attr("y", "36")
...
//        nt.on("mouseover", function() { this.focus(); });
    ...
    toJSON() {
        ...
        json += '"title": "' + this.title.node().textContent + '", ' +
                '"content": "' + this.content.node().textContent + '" }';
//        json += '"title": "' + this.group. + '", ' +
        return json;
    }
}
...
d3.select("#sketchboardjson").append("input").attr("type", "button")
            .attr("value", "Save").on("click", function () {
    var json = "{ ";
    for (var n in board.adhesives) {
        json += board.adhesives[n].toJSON() + ", ";
    }
    json = json.substring(0, json.length - 2) + " }";
    d3.select("#sketchboardjson").select("textarea").text(json);
});
