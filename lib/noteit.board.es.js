...
//TODO: use style of original text element (via CSS)..
    ...
    addHandlePhoto(slot) {
        ...
        h.on("click", function() {
            ...
            n.setFeatures(["close"], p.draggable, true, "Photo");
            n.parentObject = p;
            p.childObjects.set(n.id, n);
...
class InstantPhoto extends Adhesive {
    ...
    constructor(board, group, id="", color=["white"], geometry=[16,16], position=[2,2]) {
        super(board, group, id, color, geometry, position);
        // crate a frame (5% top/right/left; 20% bottom)
        let pictureHeight = geometry[0] * .9;
        let pictureWidth = geometry[1] * .75;
        let pictureHeightRect = pictureHeight + .1;
        let pictureWidthRect = pictureWidth + .1;
        let pictureFrame = geometry[0] * .05;
        this.group.append("rect")
                .attr("width", pictureHeightRect + "em")
                .attr("height", pictureWidthRect + "em")
                .attr("x", pictureFrame + "em")
                .attr("y", pictureFrame + "em")
                .attr("stroke", "black").attr("fill", "transparent");
        this.group.append("foreignObject")
                .attr("width", pictureHeight + "em")
                .attr("height", pictureWidth + "em")
                .attr("x", pictureFrame + "em")
                .attr("y", pictureFrame + "em")
                .append("xhtml:a").attr("href", "../example-sketchboard.png")
                .append("xhtml:img").attr("src", "../example-sketchboard.png");
        return this;
    }
