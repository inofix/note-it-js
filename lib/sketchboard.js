...
class SketchBoard {
  ...
  fromJSON(json) {
    ...
    this.createFromJSON(j)
  }

  /**
   * Create adhesives from JSON (directly from file or with fromJSON(json) above)
   **/
  createFromJSON(json) {
    try {
      this.adhesiveQueue = new Map()
      for (let k of Object.keys(json)) {
        this.adhesiveQueue.set(k, json[k])
      }
    } catch (e) {
      // TODO find a more elegant way to communicate an error than exploding in the users face..
      alert('Could not get valid data: ' + e)
    }
