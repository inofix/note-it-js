...
class Gallery {
  ...
  addImage(url) {
    if (this.allowRemote) {
      if (
        ...
        alert(
          'This installation is configured to allow remote URLs. Please use absolute URLs for all image locations.'
        )
      }
    } else {
      if (url.startsWith('/')) {
        ...
        alert(
          'Can not load image: ' + url + 'images must be on the same vHost.'
        )
      } else {
  ...
  displayImageList(svgSelector) {
    ...
    g.append('xhtml:p').text(
      'Note: if you want to restore the JSON configuration on an other installation, download the following images too:'
    )
